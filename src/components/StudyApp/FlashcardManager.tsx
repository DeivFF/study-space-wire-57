...
import { Plus, Search, Trash2, Eye, Pencil, Copy, Save, X, Sparkles, StickyNote, Layers } from 'lucide-react';
        ...
        {filteredCards.map(card => (
          <div key={card.id} className="flex items-center px-3 py-2.5 border-b border-app-border hover:bg-app-muted">
            ...
            <div className="w-32 flex justify-center gap-1">
              <Button size="sm" variant="ghost" onClick={() => handleViewCard(card)} title="Ver">
                <Eye className="w-4 h-4" />
              </Button>
              <Button size="sm" variant="ghost" onClick={() => handleEditCard(card)} title="Editar">
                <Pencil className="w-4 h-4" />
              </Button>
              <Button size="sm" variant="ghost" onClick={() => handleCloneCard(card)} title="Duplicar">
                <Copy className="w-4 h-4" />
              </Button>
              <Button size="sm" variant="ghost" onClick={() => handleDeleteCard(card.id)} title="Excluir">
                <Trash2 className="w-4 h-4" />
              </Button>
            </div>
          </div>
