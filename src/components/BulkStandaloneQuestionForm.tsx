...
interface BulkStandaloneQuestionFormProps {
  onBack: () => void;
...
const BulkStandaloneQuestionForm = ({ onBack, onQuestionsCreated }: BulkStandaloneQuestionFormProps) => {
...
Para múltiplas questões, use um array: [questão1, questão2, ...]`;
  ...
  const validateQuestion = (question: any, index: number) => {
    ...
    if (!question.materia || typeof question.materia !== 'string') {
      throw new Error('Campo "materia" é obrigatório e deve ser string');
    }

    if (!question.assunto || typeof question.assunto !== 'string') {
      throw new Error('Campo "assunto" é obrigatório e deve ser string');
    }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-700 rounded-xl p-6 text-white">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold mb-2">Adicionar Múltiplas Questões</h2>
            <p className="text-blue-100">Importe questões em formato JSON</p>
          </div>
          <Button onClick={onBack} variant="outline" className="text-blue-600 border-white hover:bg-white">
...
export default BulkStandaloneQuestionForm;
