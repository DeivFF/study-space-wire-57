...
export const LoginForm: React.FC<LoginFormProps> = ({ onForgotPassword, showSuccessMessage }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.identifier.trim()) {
      newErrors.identifier = 'Por favor, insira seu e-mail ou nome de usuário';
    }
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      if (!success) {
        setErrors({ general: 'Credenciais inválidas. Tente login/login' });
      }
    }
  };
  ...
  return (
    <form className="p-5" onSubmit={handleSubmit}>
      ...
      <div className="mb-4">
        <label className="block mb-1.5 text-sm font-medium text-app-text" htmlFor="identifier">
        ...
        <input
          ...
          className={`w-full bg-app-bg border rounded-lg p-2.5 text-sm outline-none transition-colors text-app-text ${
            errors.identifier ? 'border-app-danger' : 'border-app-border focus:border-app-accent'
          }`}
        ...
        />
        {errors.identifier && (
          <div className="text-app-danger text-xs mt-1">{errors.identifier}</div>
        )}
      </div>
