...
export const updateExercise = async (req, res) => {
  try {
    ...
    const userId = req.user.id;
    const { title, question_text, question_type, options, correct_answer, explanation, difficulty, tags, points } = req.body;
...
export const attemptExercise = async (req, res) => {
  try {
    ...
    if (exercise.question_type === 'multiple_choice' || exercise.question_type === 'mcq') {
      ...
      // No automatic correction for essays
      isCorrect = null;
    }

    // Save attempt
    await pool.query(
      'INSERT INTO exercise_attempts (exercise_id, user_id, user_answer, is_correct, time_spent) VALUES ($1, $2, $3, $4, $5)',
    ...
    await logActivity(
      ...
      timeSpent || null,
      isCorrect === true ? (exercise.points || 10) : 0,
      {
        exerciseId,
