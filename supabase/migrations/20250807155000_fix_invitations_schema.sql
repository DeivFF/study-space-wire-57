-- Add a foreign key constraint to link invitations to the inviter's profile
-- This allows joining the profile information to show the inviter's nickname.
ALTER TABLE public.trail_invitations
ADD CONSTRAINT fk_inviter_profile
FOREIGN KEY (inviter_id) REFERENCES public.profiles(user_id)
ON DELETE SET NULL;

-- Re-create the RPC function to ensure it is loaded into the schema cache and fix the "function not found" error.
